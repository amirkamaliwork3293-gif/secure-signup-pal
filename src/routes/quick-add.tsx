...
function QuickAdd() {
  ...
  const update = (id: string, k: keyof Draft, v: string) =>
    setDrafts((p) => p.map((d) => (d.id === id ? { ...d, [k]: v } : d)));
  const remove = (id: string) =>
    setDrafts((p) => (p.length > 1 ? p.filter((d) => d.id !== id) : p));

  const submit = (andPrint: boolean) => {
    const valid = drafts.filter(
      (d) => d.name.trim() && fromDisplayAmount(parseNumberInput(d.price)) > 0,
    );
    if (valid.length === 0) {
      alert("حداقل یک محصول با نام و قیمت وارد کنید.");
      return;
    }
    ...
    const created: Product[] = valid.map((d) => ({
      id: cryptoId(),
      name: d.name.trim(),
      price: fromDisplayAmount(parseNumberInput(d.price)),
      code: generateUniqueCode(taken),
      stock: parseNumberInput(d.stock) || 0,
      category: "",
    }));
    ...
    <Layout>
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            نام، قیمت ({currencyLabel()}) و موجودی — بارکد یکتا خودکار تولید می‌شود
          </p>
        </div>
        <Link
          to="/products"
      ...
      <div className="space-y-2">
        {drafts.map((d, i) => (
          ...
          >
            <span className="grid h-9 w-7 place-items-center text-xs text-muted-foreground">
            ...
            <input
              ...
              inputMode="numeric"
              placeholder={`قیمت (${currencyLabel()})`}
      ...
      <button
        onClick={add}
        className="mt-2 w-full inline-flex items-center justify-center gap-1 rounded-xl border border-dashed border-border px-4 py-2.5 text-xs text-muted-foreground hover:bg-accent"
      >
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        ...
        <button
          onClick={() => submit(true)}
          className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-primary px-4 py-3 text-sm font-medium text-primary-foreground"
        >
