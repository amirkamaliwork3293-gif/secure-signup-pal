...
DO $$ BEGIN CREATE TYPE public.profile_status AS ENUM ('pending','active','expired','rejected'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.subscription_plan AS ENUM ('1month','3month','6month'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE POLICY "anyone_create_signup_request" ON public.signup_requests FOR INSERT TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "anyone_read_own_request_by_username" ON public.signup_requests;
CREATE POLICY "anyone_read_own_request_by_username" ON public.signup_requests FOR SELECT TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "admins_update_requests" ON public.signup_requests;
CREATE POLICY "admins_update_requests" ON public.signup_requests FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'admin'));
CREATE INDEX IF NOT EXISTS signup_requests_username_idx ON public.signup_requests (username);
...
CREATE TABLE IF NOT EXISTS public.app_settings (
  ...
  price_1month bigint NOT NULL DEFAULT 100000,
  price_3month bigint NOT NULL DEFAULT 280000,
  price_6month bigint NOT NULL DEFAULT 500000,
...
CREATE POLICY "anyone_read_settings" ON public.app_settings FOR SELECT TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "admins_update_settings" ON public.app_settings;
CREATE POLICY "admins_update_settings" ON public.app_settings FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "users_select_own_data" ON public.user_data FOR SELECT TO authenticated USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "users_insert_own_data" ON public.user_data;
CREATE POLICY "users_insert_own_data" ON public.user_data FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "users_update_own_data" ON public.user_data;
CREATE POLICY "users_update_own_data" ON public.user_data FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "users_delete_own_data" ON public.user_data;
CREATE POLICY "users_delete_own_data" ON public.user_data FOR DELETE TO authenticated USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "admins_read_user_data" ON public.user_data;
CREATE POLICY "admins_read_user_data" ON public.user_data FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));