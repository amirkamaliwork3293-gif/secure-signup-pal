...
-- راهنمای ساده: docs/RAHNAMA-AMNIAT.md  (مرحله ۳)
--
-- چه می‌کند:
--   • سقف ثبت‌نام را درست می‌کند تا کاربر واقعی قفل نشود
--   • دسترسی‌های باز دیتابیس را می‌بندد
--   • هکر را از حساب‌های باز بیرون می‌کند (شما هم خارج می‌شوید — عمدی است)
-- بعد از Run پایین صفحه باید rate_limit_ok = true باشد و فقط ادمین خودتان در فهرست باشد.
-- ════════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════════
-- بیرون کردن هکر از حساب‌ها
-- بعد از این دو خط، همه (از جمله خود شما) از حساب خارج می‌شوند. این خوب است.
-- ════════════════════════════════════════════════════════════════════════════
...
-- نتیجه را ببینید
-- ۱) باید true باشد
-- ۲) در فهرست ادمین فقط حساب خودتان باید باشد. اگر ردیف ناشناس دیدید به من بگویید.
-- ════════════════════════════════════════════════════════════════════════════
SELECT public.check_rate_limit('smoke-test', 5, 60) AS rate_limit_ok;
