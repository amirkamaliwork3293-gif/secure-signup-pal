...
export const Route = createFileRoute("/store-qr")({
  head: () => ({ meta: [{ title: "QR کد فروشگاه | KAMIX" }] }),
  component: () => (
    <AuthGuard>
      <StoreQrPage />
    </AuthGuard>
...
function StoreQrPage() {
  ...
  const url = userId ? storePublicUrl(userId) : "";

  useEffect(() => {
    if (!url || !canvasRef.current) return;
    // مثل QR منو: پیشوند صریح https:// تا اسکنر دوربین گوشی آن را به‌عنوان لینک
    // تشخیص دهد (نه متن ساده).
  ...
  const doPrint = async () => {
    if (!dataUrl) return;
    setWorking(true);
    await printHtml(buildHtml(), `${shopName}-store-qr`);
    setWorking(false);
  };

  const downloadPng = async () => {
    if (!dataUrl) return;
    await saveBase64File(dataUrl, `${shopName}-store-qr.png`, "image/png");
  };
  ...
  const downloadPdf = async () => {
    ...
    pdf.setFontSize(10);
    pdf.text("Scan for store page", pdf.internal.pageSize.getWidth() / 2, 40 + sizeMm + 10, { align: "center" });
    await savePdf(pdf as any, `${shopName}-store-qr.pdf`);
    setWorking(false);
  };
    ...
    <Layout>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="flex items-center gap-2 text-lg font-bold">
          ...
          QR کد پروفایل فروشگاه
        </h1>
        <Link to="/settings" className="flex items-center gap-1 rounded-xl border border-border px-3 py-2 text-xs">
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        <div className="mb-4 text-xs text-muted-foreground">
          این QR کد را پرینت و در مغازه بچسبانید تا مشتری با اسکن آن، صفحه‌ی معرفی فروشگاه شما
          (آدرس، تلفن، ساعات کاری و شبکه‌های اجتماعی) را ببیند.
        </div>
        {!url ? (
          <div className="rounded-xl border border-dashed border-border bg-background p-4 text-center text-xs text-muted-foreground">
            ابتدا از تنظیمات، «پروفایل عمومی فروشگاه» را ذخیره و منتشر کنید.
          </div>
        ) : (
          <>
